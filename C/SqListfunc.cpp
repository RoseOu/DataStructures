...
void GetElem_Sq(SqList L,int i,ElemType &e)     //用e返回第i个元素的值
{
	e=L.elem[i-1];
}
...
void PriorElem_Sq(SqList L,ElemType cur_e,ElemType &pre_e)   //若cur_e不是第1个元素，则返回它的前驱
{
    int i;
    ElemType *p;
	i=1;             //i的初值为第1个元素的位序
...
void increment(SqList &L)     //用于扩容的函数，扩大incrementsize个元素空间
{
	int i;
	ElemType *a;
